...
date: "2018-07-09T07:15:59+05:30"
publishdate: "2018-07-09T07:15:59+05:30"
...
Often, it is also lack of exposure to CSS selectors. People have mentioned that they are able to get the job done with other selectors. Fair enough, no harm done.
...
Just like that, there are ways to identify elements in a HTML document/DOM. But CSS selectors are the tool for the job. They have a [specification that browsers need to follow](https://www.w3.org/TR/selectors-3/)

If you are an automation engineer working on UI automation, you'll reap benefits that are several-fold more than the time you invest in learning CSS selectors.

I can hear your question: "But, it is so easy to copy the xPath. How about css selectors?" Trust me, in many browsers, the option is right there beside `Copy xPath`. You'll find `Copy CSS Selector` or `Copy Selector`. 
...
Selectors to target specific elements was specified formally by [CSS Working Group](https://www.w3.org/Style/CSS/members). The browser developers take that specification as requirement and build the browsers to support CSS.

Attributes such as `id` and `class` were added as hooks to target specific elements. They are there to help front end developers style and attach behaviors to elements. **Automation engineers can piggyback** on the same provision to identify elements that are used by front-end engineers.
...
Usually, ID is unique. It's a fair assumption that there's usually one. Assumptions can go wrong. Twenty developers working on a very busy UI might end up slipping the same ID on two different elements. So, if you wanted to pair it with a tag, you can do so. Put the tag name in front of the ID.
...
That will give you the first table in the page. 
...
It is the right most element that is selected. For example, while using `h1 + p`, the `p` element is selected. `h1` is there as a supporting actor.
...
If you wanted to access all images that are sibling of a `h1` tag, you can use the general sibling combinator. If there is only one `img`, you'll get just that.

```js
By.cssSelector('h1 ~ img')
```
In this instance, it doesn't matter if the `img` tags follow `h1` immediately. What matters is, they all have the same parent and `img` tags are sibling to `h1`.
...
We are now down to the fourth and last combinator, the descendant combinator which is desingnated by a white space ` `.
...
You see the difference. Descendant combinator selects all matching tags as long as they are descenants of the parent element given.
...
You could have used the right tag name instead of `*`, as in `#profile > img:nth-child(2)`. You need to be careful when the tag name does not match. For example, no element will be selected if you use a tag name that is not there. For example, `#profile > h1:nth-child(2)` returns nothing, as the second child is not a `h1` tag.

`:nth-child` takes input like a number, `odd`, `even` and `3n+2`. You can read `3n+2` like, every 3rd element starting from 2nd element. MDN defines it like `An+B`. This is useful more in terms of styling elements. May not be so much for test automation. Let's leave it right there. 

There is another one to get an element counting from bottom: `nth-last-child`. For example, `#profile > *:nth-last-child(1)` will give you the last child, which is `p` tag that has the text "Conclusion...". The spec has `:last-child` as a pseudo class that will give you similar outcome.
...
What do you do to count from bottom (reverse order)? Yeah, `:nth-last-of-type` will come to your help. You also have `:last-of-type` to get the bottom most child of specified type.
...
In my experience, test automation engineers started using `xpath` universally. So, this article was to add counterweight to `cssSelectors` so that the scale is balanced against `xPath`. In fact, John Resig, the creator of jQuery has written about such a [comparison showing how powerful xPath][john-resig] can be, this was in 2005.
